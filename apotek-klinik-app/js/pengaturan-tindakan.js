 ...
 * Master data tindakan klinik & apotek — CRUD lengkap
 * Setiap tindakan punya hargaJual, modal (fixed), dan consumable opsional
 */
...
window.AppPengaturanTindakan = {
    ...
    setFilter: function(kat) {
        AppPengaturanTindakan.filterKategori = kat;
    ...
    load: function() {
        ...
        db.collection('masterTindakan').orderBy('kategori').orderBy('nama').get()
            .then(function(snap) {
    ...
    renderList: function() {
        ...
        if (AppPengaturanTindakan.filterKategori !== 'semua') {
            list = list.filter(function(t) {
                return t.kategori === AppPengaturanTindakan.filterKategori;
            });
        }

        if (list.length === 0) {
            container.innerHTML = '<div class="bg-white rounded-xl border border-gray-200 p-8 text-center"><p class="text-sm text-gray-400">Belum ada data tindakan</p></div>';
            return;
        }
        ...
        html += '<div class="col-span-2">Kategori</div>';
        html += '<div class="col-span-2">Harga Jual</div>';
        html += '<div class="col-span-2">Modal</div>';
        html += '<div class="col-span-1">Tuslah</div>';
        ...
        for (var i = 0; i < list.length; i++) {
            ...
            var namaEsc = Utils.escapeHtml(t.nama || '-');
            var safeName = (t.nama || '-').replace(/\\/g, '\\\\').replace(/'/g, "\\'");
            var katBadge = (t.kategori === 'klinik')
                ? '<span class="text-xs bg-purple-50 text-purple-700 px-2 py-0.5 rounded-full">Klinik</span>'
                : '<span class="text-xs bg-teal-50 text-teal-700 px-2 py-0.5 rounded-full">Apotek</span>';
            var aktifBadge = (t.aktif !== false)
                ? '<span class="text-xs bg-green-50 text-green-700 px-2 py-0.5 rounded-full ml-1">Aktif</span>'
                : '<span class="text-xs bg-gray-100 text-gray-500 px-2 py-0.5 rounded-full ml-1">Nonaktif</span>';
            ...
            html += '<div class="col-span-2">' + katBadge + aktifBadge + '</div>';
            html += '<div class="col-span-2 text-gray-600">' + Utils.formatRupiah(t.hargaJual) + '</div>';
            html += '<div class="col-span-2 text-gray-600">' + Utils.formatRupiah(t.modal) + '</div>';
            html += '<div class="col-span-1 text-green-600 font-medium">' + Utils.formatRupiah(tuslah) + '</div>';
            html += '<div class="col-span-2 flex justify-end gap-1">';
            html += '<button onclick="AppPengaturanTindakan.renderFormEdit(\'' + t.id + '\')" class="p-1.5 text-gray-400 hover:text-primary-600 hover:bg-primary-50 rounded-lg" title="Edit"><i data-lucide="pencil" class="w-4 h-4"></i></button>';
            html += '<button onclick="AppPengaturanTindakan.hapus(\'' + t.id + '\',\'' + safeName + '\')" class="p-1.5 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-lg" title="Hapus"><i data-lucide="trash-2" class="w-4 h-4"></i></button>';
            html += '</div></div>';
            ...
            html += '<p class="text-xs text-gray-500 mt-0.5">Jual: ' + Utils.formatRupiah(t.hargaJual) + ' · Modal: ' + Utils.formatRupiah(t.modal) + ' · Tuslah: ' + Utils.formatRupiah(tuslah) + '</p>';
            html += '</div>';
            html += '<div class="flex items-center gap-2 flex-shrink-0">';
            html += katBadge;
            html += '<button onclick="AppPengaturanTindakan.renderFormEdit(\'' + t.id + '\')" class="p-1.5 text-gray-400 hover:text-primary-600 rounded-lg"><i data-lucide="pencil" class="w-4 h-4"></i></button>';
            html += '<button onclick="AppPengaturanTindakan.hapus(\'' + t.id + '\',\'' + safeName + '\')" class="p-1.5 text-gray-400 hover:text-red-600 rounded-lg"><i data-lucide="trash-2" class="w-4 h-4"></i></button>';
            html += '</div></div></div>';
    ...
    renderFormTambah: function() {
        AppPengaturanTindakan.formConsumables = [];
        AppPengaturanTindakan.renderForm(null);
    },

    renderFormEdit: function(id) {
        var tindakan = null;
        for (var i = 0; i < AppPengaturanTindakan.data.length; i++) {
            if (AppPengaturanTindakan.data[i].id === id) {
                tindakan = AppPengaturanTindakan.data[i];
                break;
            }
        }
        if (!tindakan) {
            Utils.toast('Data tidak ditemukan', 'error');
            return;
        }
    ...
    renderForm: function(data) {
        var isEdit = !!data;
        var title = isEdit ? 'Edit Tindakan' : 'Tambah Tindakan';
        var v = data || {};
        ...
        // Nama
        html += '<div>';
        html += '<label class="block text-sm font-medium text-gray-700 mb-1">Nama Tindakan *</label>';
        html += '<input type="text" id="ft-nama" value="' + Utils.escapeHtml(v.nama || '') + '" required class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm" placeholder="Cek Gula Darah">';
        html += '</div>';
        ...
        html += '<select id="ft-kategori" required class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm">';
        html += '<option value="">-- Pilih --</option>';
        html += '<option value="klinik"' + (v.kategori === 'klinik' ? ' selected' : '') + '>Klinik</option>';
        html += '<option value="apotek"' + (v.kategori === 'apotek' ? ' selected' : '') + '>Apotek</option>';
        html += '</select></div>';
        html += '<div>';
        html += '<label class="block text-sm font-medium text-gray-700 mb-1">Status</label>';
        html += '<select id="ft-aktif" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm">';
        html += '<option value="true"' + (v.aktif !== false ? ' selected' : '') + '>Aktif</option>';
        html += '<option value="false"' + (v.aktif === false ? ' selected' : '') + '>Nonaktif</option>';
        html += '</select></div></div>';

        // Harga Jual + Modal
        html += '<div class="grid grid-cols-2 gap-4">';
        html += '<div>';
        html += '<label class="block text-sm font-medium text-gray-700 mb-1">Harga Jual (Rp) *</label>';
        html += '<input type="number" id="ft-harga-jual" value="' + (v.hargaJual || 0) + '" required min="0" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm" placeholder="15000" oninput="AppPengaturanTindakan.previewTuslah()">';
        html += '</div>';
        html += '<div>';
        html += '<label class="block text-sm font-medium text-gray-700 mb-1">Modal / Biaya Bahan (Rp) *</label>';
        html += '<input type="number" id="ft-modal" value="' + (v.modal || 0) + '" required min="0" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm" placeholder="13000" oninput="AppPengaturanTindakan.previewTuslah()">';
        html += '<p id="ft-tuslah-info" class="text-xs text-green-600 mt-1 font-medium"></p>';
        html += '</div></div>';
        ...
        if (isEdit) {
            html += '<input type="hidden" id="ft-id" value="' + v.id + '">';
        }
        ...
        // Render existing consumables
        for (var c = 0; c < AppPengaturanTindakan.formConsumables.length; c++) {
            AppPengaturanTindakan.renderConsumableRow(c);
        }
    ...
    previewTuslah: function() {
        ...
        if (el) {
            el.textContent = 'Tuslah: ' + Utils.formatRupiah(tuslah);
        }
    },
    ...
    /* =========================================
       ...
       ========================================= */
    tambahConsumable: function() {
        var index = AppPengaturanTindakan.formConsumables.length;
        AppPengaturanTindakan.formConsumables.push({
            obatId: '',
            namaObat: '',
            jumlah: 1
        });
    ...
    renderConsumableRow: function(index) {
        var container = document.getElementById('ft-consumables-container');
        if (!container) return;
        ...
        html += '<input type="text" id="ftc-search-' + index + '" value="' + Utils.escapeHtml(item.namaObat) + '" placeholder="Ketik nama obat/alkes..." class="w-full pl-10 pr-3 py-2 border border-gray-300 rounded-lg text-sm" autocomplete="off" onfocus="AppPengaturanTindakan.filterConsumableDropdown(' + index + ')" oninput="AppPengaturanTindakan.filterConsumableDropdown(' + index + ')">';
        html += '<div id="ftc-dropdown-' + index + '" class="hidden absolute z-20 left-0 right-0 mt-1 bg-white border border-gray-200 rounded-lg shadow-lg max-h-40 overflow-y-auto"></div>';
        html += '</div>';

        html += '<div class="w-32">';
        html += '<label class="block text-xs text-gray-500 mb-0.5">Jumlah</label>';
        html += '<input type="number" id="ftc-jumlah-' + index + '" value="' + (item.jumlah || 1) + '" min="1" class="w-full px-2 py-1.5 border border-gray-300 rounded-lg text-xs" oninput="AppPengaturanTindakan.updateConsumableJumlah(' + index + ')">';
        html += '</div>';
    ...
    filterConsumableDropdown: function(index) {
        var dropdown = document.getElementById('ftc-dropdown-' + index);
        if (!dropdown) return;

        var searchEl = document.getElementById('ftc-search-' + index);
        if (!searchEl) return;
        ...
        for (var i = 0; i < results.length; i++) {
            var o = results[i];
            html += '<button type="button" onclick="AppPengaturanTindakan.selectConsumable(' + index + ',\'' + o.id + '\')" class="w-full text-left px-3 py-2 hover:bg-primary-50 text-sm border-b border-gray-50 last:border-0 transition">';
            html += '<p class="font-medium text-gray-800">' + Utils.escapeHtml(o.namaObat) + '</p>';
            html += '<p class="text-xs text-gray-500">Stok: ' + (o.stok || 0) + ' ' + Utils.escapeHtml(o.satuan || '') + '</p>';
            html += '</button>';
        }
    ...
    selectConsumable: function(index, obatId) {
        ...
        var oldRow = document.getElementById('ftc-' + index);
    ...
    updateConsumableJumlah: function(index) {
        ...
        if (el) {
            AppPengaturanTindakan.formConsumables[index].jumlah = parseInt(el.value) || 1;
        }
    },

    hapusConsumable: function(index) {
        var row = document.getElementById('ftc-' + index);
        if (row) row.remove();
        AppPengaturanTindakan.formConsumables[index] = null;
    },
    ...
    /* =========================================
       ...
       ========================================= */
    simpan: function() {
        var btn = document.getElementById('btn-simpan-tindakan');
        btn.disabled = true;
        ...
        if (!nama) {
            Utils.toast('Nama tindakan wajib diisi', 'error');
            btn.disabled = false;
            btn.textContent = 'Simpan';
            return;
        }

        // Kumpulkan consumables yang valid
        ...
        var promise;
        if (isEdit) {
            promise = db.collection('masterTindakan').doc(id).update(obj);
        } else {
            obj.createdAt = firebase.firestore.FieldValue.serverTimestamp();
            promise = db.collection('masterTindakan').add(obj);
        }

        promise.then(function() {
            Utils.toast(isEdit ? 'Tindakan berhasil diperbarui' : 'Tindakan berhasil ditambahkan', 'success');
            Utils.closeModal();
            AppPengaturanTindakan.load();
        }).catch(function(err) {
    ...
    hapus: function(id, nama) {
        ...
        db.collection('masterTindakan').doc(id).delete()
            .then(function() {
                Utils.toast('Tindakan berhasil dihapus', 'success');
                AppPengaturanTindakan.load();
            })
            .catch(function(err) {
    ...
    getById: function(id) {
        for (var i = 0; i < AppPengaturanTindakan.data.length; i++) {
            if (AppPengaturanTindakan.data[i].id === id) {
                return AppPengaturanTindakan.data[i];
            }
        }
        return null;
    },
    ...
    getByKategori: function(kat) {
        return AppPengaturanTindakan.data.filter(function(t) {
            return t.kategori === kat && t.aktif !== false;
        });
    }
};
