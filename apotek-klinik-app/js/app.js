...
// Inisialisasi Firebase dengan config yang lu kasi
var firebaseConfig = {
...
// Global app state
window.App = {
    currentUser: null,   // data user dari Firestore
    currentPage: '',     // halaman yang sedang aktif
    openMenus: {},       // track sidebar submenu yang terbuka
    pengaturan: null,    // data pengaturan global (cache)
    ...
    init: function() {
        ...
        auth.onAuthStateChanged(function(user) {
            if (user) {
                // User sudah login → load data user dari Firestore
                App.loadUser(user.uid);
            } else {
                // User belum login → cek first time atau tampilkan login
                App.showLoginPage();
            }
        });

        // Dengarkan perubahan hash URL untuk routing
        ...
        db.collection('users').doc(uid).get()
            .then(function(doc) {
                if (doc.exists) {
                    ...
                    // Biasanya terjadi kalau setup belum selesai
    ...
    // ===========================
    // MASUK KE APP (setelah login)
    // ===========================
    enterApp: function() {
        // Sembunyikan login, tampilkan shell
        ...
        // Load pengaturan global (untuk nama di sidebar)
        App.loadPengaturan();

        // Build sidebar menu sesuai role
        App.buildSidebar();

        // Route ke halaman default
    ...
    loadPengaturan: function() {
        db.collection('pengaturan').doc('global').get()
            .then(function(doc) {
                if (doc.exists) {
    ...
    toggleMenu: function(menuId) {
        ...
        App.buildSidebar(); // rebuild sidebar
    },
    ...
    route: function() {
        ...
        // Rebuild sidebar untuk update active state
        App.buildSidebar();
        ...
        if (modul && modul.render) {
            ...
            content.innerHTML = ''
                + '<div class="page-enter flex flex-col items-center justify-center py-20">'
                + '  <div class="w-20 h-20 bg-gray-100 rounded-2xl flex items-center justify-center mb-4">'
                + '    <i data-lucide="construction" class="w-10 h-10 text-gray-400"></i>'
                + '  </div>'
                + '  <h3 class="text-lg font-semibold text-gray-600">' + title + '</h3>'
                + '  <p class="text-sm text-gray-400 mt-1">Module ini sedang dalam pengembangan</p>'
                + '</div>';
            lucide.createIcons();
        }
    },
