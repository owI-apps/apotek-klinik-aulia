...
window.AppRekamMedis = {
    ...
    loadForForm: function(antrianId) {
        db.collection('antrian').doc(antrianId).get().then(function(doc) {
            if (!doc.exists) {
            ...
            var pasienData = null;
            var dokterData = null;
            var p1 = db.collection('pasien').doc(antrian.pasienId).get().then(function(pd) { if (pd.exists) pasienData = pd.data(); });
            var p2 = db.collection('users').doc(antrian.dokterId).get().then(function(dd) { if (dd.exists) dokterData = dd.data(); });
    ...
    renderForm: function(prefill) {
        ...
        if (isFromAntrian) {
            ...
            html += '<div><label class="block text-sm font-medium text-gray-700 mb-1">Pasien</label><input type="text" id="rm-pasien-nama" value="' + Utils.escapeHtml(v.pasienNama || '') + '" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm bg-gray-50" readonly></div>';
            html += '<div><label class="block text-sm font-medium text-gray-700 mb-1">Dokter</label><input type="text" id="rm-dokter-nama" value="' + Utils.escapeHtml(v.dokterNama || App.currentUser.nama) + '" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm bg-gray-50" readonly></div>';
            html += '</div>';
            html += '<input type="hidden" id="rm-antrian-id" value="">';
        }
        ...
        html += '<div><label class="block text-sm font-medium text-gray-700 mb-1">Keluhan *</label><textarea id="rm-keluhan" rows="2" required class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm" placeholder="Keluhan pasien...">' + Utils.escapeHtml(v.keluhan || '') + '</textarea></div>';
        html += '<div><label class="block text-sm font-medium text-gray-700 mb-1">Diagnosa</label><input type="text" id="rm-diagnosa" value="' + Utils.escapeHtml(v.diagnosa || '') + '" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm" placeholder="Diagnosa..."></div>';
        html += '<div><label class="block text-sm font-medium text-gray-700 mb-1">Tindakan</label><input type="text" id="rm-tindakan" value="' + Utils.escapeHtml(v.tindakan || '') + '" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm" placeholder="Pemeriksaan fisik, dll..."></div>';
        html += '<div><label class="block text-sm font-medium text-gray-700 mb-1">Catatan</label><textarea id="rm-catatan" rows="2" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm" placeholder="Catatan tambahan...">' + Utils.escapeHtml(v.catatan || '') + '</textarea></div>';
    ...
    simpan: function() {
        ...
        var savePromise;
        if (hasAntrian) {
            savePromise = db.collection('antrian').doc(antrianIdField.value).get().then(function(antrianDoc) {
