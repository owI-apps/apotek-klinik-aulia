/**
 * antrian.js
 * Antrian harian — daftar, panggil, selesai
 */
...
window.AppAntrian = {
    ...
    /* =========================================
       ...
       ========================================= */
    loadDokter: function() {
        db.collection('users').where('role', '==', 'klinik').get()
            .then(function(snap) {
                AppAntrian.dokterList = [];
                snap.forEach(function(doc) {
                    var d = doc.data();
                    d.id = doc.id;
                    AppAntrian.dokterList.push(d);
                });
            })
            .catch(function() {});
    },
    ...
    load: function() {
        ...
        db.collection('antrian').where('tanggal', '==', today).orderBy('noAntrian').get()
            .then(function(snap) {
                AppAntrian.data = [];
                snap.forEach(function(doc) {
                    var d = doc.data();
                    d.id = doc.id;
                    AppAntrian.data.push(d);
                });
                AppAntrian.renderList();
            })
            .catch(function(err) {
    ...
    renderList: function() {
        ...
        for (var i = 0; i < list.length; i++) {
            ...
            var badgeClass = statusColors[a.status] || 'bg-gray-100 text-gray-600';
            ...
            html += '<span class="text-xs px-2 py-0.5 rounded-full font-medium ' + badgeClass + '">' + Utils.escapeHtml(a.status || '-') + '</span>';
            html += '</div>';
            ...
            if (a.status === 'selesai') {
                html += '<span class="text-xs text-gray-400">Selesai diproses</span>';
            }
            html += '</div>';
    ...
    renderFormTambah: function() {
        if (AppAntrian.dokterList.length === 0) {
            Utils.toast('Belum ada dokter terdaftar. Tambahkan di Pengaturan > Kelola Users.', 'error');
            return;
        }
        ...
        html += '<input type="text" id="fa-pasien-search" placeholder="Ketik nama pasien..." class="w-full pl-10 pr-4 py-2.5 border border-gray-300 rounded-lg text-sm" autocomplete="off">';
        html += '</div>';
        ...
        html += '<div id="fa-pasien-selected" class="hidden mt-2 bg-primary-50 border border-primary-200 rounded-lg px-3 py-2 flex items-center justify-between">';
        html += '<span id="fa-pasien-selected-name" class="text-sm font-medium text-primary-800"></span>';
        html += '<button type="button" onclick="AppAntrian.clearPasienSelection()" class="text-primary-400 hover:text-primary-600 text-xs font-medium">Ganti</button>';
        html += '</div>';
        html += '</div>';
        html += '</div>';
        ...
        html += '<div>';
        html += '<label class="block text-xs font-medium text-gray-600 mb-1">Nama *</label>';
        html += '<input type="text" id="fa-nb-nama" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm" placeholder="Nama pasien">';
        html += '</div>';
        html += '<div>';
        html += '<label class="block text-xs font-medium text-gray-600 mb-1">No. Telepon</label>';
        html += '<input type="text" id="fa-nb-telp" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm" placeholder="08xxx">';
        html += '</div>';
        html += '<div>';
        html += '<label class="block text-xs font-medium text-gray-600 mb-1">Jenis Kelamin</label>';
        html += '<select id="fa-nb-jk" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm">';
        ...
        html += '<select id="fa-dokter" required class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm">';
        html += '<option value="">-- Pilih Dokter --</option>';
        for (var d = 0; d < AppAntrian.dokterList.length; d++) {
            var dok = AppAntrian.dokterList[d];
            html += '<option value="' + dok.id + '">' + Utils.escapeHtml(dok.nama) + '</option>';
        }
        html += '</select></div>';
        ...
        searchInput.addEventListener('input', function() {
            ...
            searchTimeout = setTimeout(function() {
                AppAntrian.filterPasienDropdown(val);
            }, 200);
        });

        searchInput.addEventListener('focus', function() {
            if (searchInput.value.trim()) {
                AppAntrian.filterPasienDropdown(searchInput.value.trim().toLowerCase());
            }
        });
        ...
        // Event: submit form
        document.getElementById('form-antrian').addEventListener('submit', function(e) {
            e.preventDefault();
            AppAntrian.simpan();
        });
    },
    ...
    /* =========================================
       PASIEN BARU TOGGLE
       ========================================= */
    togglePasienBaru: function() {
        var form = document.getElementById('fa-pasien-baru-form');
        if (form) {
            form.classList.toggle('hidden');
            if (!form.classList.contains('hidden')) {
                document.getElementById('fa-nb-nama').focus();
            }
        }
    },
    ...
    filterPasienDropdown: function(query) {
        ...
        // Pastikan data pasien sudah loaded, kalau belum load dulu
        if (AppPasien.data.length === 0) {
            db.collection('pasien').orderBy('nama').limit(50).get()
                .then(function(snap) {
                    AppPasien.data = [];
                    snap.forEach(function(doc) {
                        var d = doc.data();
        ...
        var results = AppPasien.data.filter(function(p) {
            return (p.nama && p.nama.toLowerCase().indexOf(query) !== -1)
                || (p.noTelp && p.noTelp.indexOf(query) !== -1);
        }).slice(0, 8);
        ...
        for (var i = 0; i < results.length; i++) {
            var p = results[i];
            html += '<button type="button" onclick="AppAntrian.selectPasien(\'' + p.id + '\')" class="w-full text-left px-3 py-2 hover:bg-primary-50 text-sm border-b border-gray-50 last:border-0 transition">';
            html += '<p class="font-medium text-gray-800">' + Utils.escapeHtml(p.nama) + '</p>';
            html += '<p class="text-xs text-gray-500">' + Utils.escapeHtml(p.noTelp || '-') + ' · ' + Utils.escapeHtml(p.jenisKelamin || '-') + '</p>';
            html += '</button>';
        }
    ...
    simpan: function() {
        ...
        if (!dokterId) {
            Utils.toast('Pilih dokter', 'error');
            btn.disabled = false;
        ...
        var dokterNama = '';
        for (var d = 0; d < AppAntrian.dokterList.length; d++) {
            if (AppAntrian.dokterList[d].id === dokterId) {
                dokterNama = AppAntrian.dokterList[d].nama;
                break;
            }
        }

        // Step 1: Kalau pasien baru, buat dulu
        var step1;
        if (isPasienBaru) {
            var namaBaru = document.getElementById('fa-nb-nama').value.trim();
            if (!namaBaru) {
                Utils.toast('Nama pasien baru wajib diisi', 'error');
                btn.disabled = false;
                btn.textContent = 'Daftar';
                return;
            }
            var pasienObj = {
                nama: namaBaru,
                noTelp: document.getElementById('fa-nb-telp').value.trim(),
                jenisKelamin: document.getElementById('fa-nb-jk').value,
            ...
            Utils.toast('Pilih atau buat pasien', 'error');
            btn.disabled = false;
            btn.textContent = 'Daftar';
            return;
        }

        // Step 2: Setelah ada pasienId, buat antrian
        step1.then(function(pasienId) {
            ...
            if (isPasienBaru) {
                ...
                for (var p = 0; p < AppPasien.data.length; p++) {
                    if (AppPasien.data[p].id === pasienId) {
                        pasienNama = AppPasien.data[p].nama;
                        break;
                    }
                }
            }
            ...
            var today = Utils.today();
            return db.collection('antrian').where('tanggal', '==', today).get()
                .then(function(snap) {
                    var nomor = snap.size + 1;
                    var noAntrian = 'A-' + String(nomor).padStart(3, '0');

                    return db.collection('antrian').add({
                        noAntrian: noAntrian,
                        tanggal: today,
                        pasienId: pasienId,
            ...
            Utils.toast('Antrian berhasil didaftarkan', 'success');
            Utils.closeModal();
            AppAntrian.load();
            // Refresh pasien list kalau ada pasien baru
            if (isPasienBaru) AppPasien.load();
        }).catch(function(err) {
            Utils.toast('Gagal mendaftarkan: ' + err.message, 'error');
            btn.disabled = false;
            btn.textContent = 'Daftar';
        });
    },
    ...
    /* =========================================
       ...
       ========================================= */
    panggil: function(id) {
        db.collection('antrian').doc(id).update({ status: 'diperiksa' })
            .then(function() {
    ...
    selesai: function(id) {
        db.collection('antrian').doc(id).update({ status: 'selesai' })
            .then(function() {
    ...
    hapus: function(id) {
        ...
        db.collection('antrian').doc(id).delete()
            .then(function() {
                Utils.toast('Antrian dibatalkan', 'success');
                AppAntrian.load();
            })
            .catch(function(err) {
                Utils.toast('Gagal: ' + err.message, 'error');
    ...
    /* =========================================
       ...
       ========================================= */
    buatRekamMedis: function(antrianId) {
        window.location.hash = '#rekam-medis?antrian=' + antrianId;
    }
};
